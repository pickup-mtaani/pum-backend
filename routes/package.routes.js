    ...
    if (body.delivery_type === "door_step") {
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        if (packages[i].products?.length !== 0) {
          ...
          let products_name = item
            ?.map(function (item) {
              return `${item.product_name}(${item?.cart_amt})`;
            })
            .join(",");
          let products_price = item?.reduce(function (
            accumulator,
            currentValue
          ) {
            const totalPrice =
              parseInt(currentValue.price) * parseInt(currentValue?.cart_amt);
          ...
          for (let k = 0; k < item.length; k++) {
            ...
            await Product.findOneAndUpdate(
              { _id: item[k]._id },
              { qty: parseInt(product.qty) - parseInt(item[k].cart_amt) },
              { new: true, useFindAndModify: false }
            );
          }
        ...
        let customer = await Customer.findOne({
          seller: req.user._id,
          customer_phone_number: packages[i].customerPhoneNumber,
        });
        if (customer === null) {
          await new Customer({
            door_step_package_count: 1,
            total_package_count: 1,
            seller: req.user._id,
            customer_name: packages[i].customerName,
            customer_phone_number: packages[i].customerPhoneNumber,
          }).save();
        } else {
          await Customer.findOneAndUpdate(
            { seller: req.user._id },
            {
              door_step_package_count: parseInt(
                customer.door_step_package_count + 1
              ),
              total_package_count: parseInt(customer.total_package_count + 1),
            },
            { new: true, useFindAndModify: false }
          );
        }
        ...
        await new Notification({
          dispachedTo: packages[i].createdBy,
          receipt_no: `${packages[i].receipt_no}`,
          p_type: 2,
          s_type: 1,
          descriptions: ` Package #${packages[i].receipt_no}  created`,
        }).save();
        await new Track_door_step({
          ...
          descriptions: [
            {
              time: moment(),
              descriptions: `Pkg ${packages[i].receipt_no} created by ${business.name}`,
            },
          ],
        }).save();
        await AgentDetails.findOneAndUpdate(
          { _id: packages[i].agent },
          { package_count: newPackageCount },
          { new: true, useFindAndModify: false }
        );
      }

      return res.status(200).json(savedPackages);
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        if (packages[i].other) {
          let data = {
            name: packages[i].other,
            descriptions: packages[i].other,
          };
        ...
        if (customer === null) {
          ...
          await Customer.findOneAndUpdate(
            { seller: req.user._id },
            {
              erands_package_count: parseInt(customer.erands_package_count + 1),
              total_package_count: parseInt(customer.total_package_count + 1),
            },
            { new: true, useFindAndModify: false }
          );
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        let agent_id = await AgentDetails.findOne({
          _id: business.shelf_location,
        });
        ...
        packages[i].receipt_no = `PMT-RTF-${parseInt(
          agent_id?.package_count + 1
        )}`;
        packages[i].createdAt = moment().format("YYYY-MM-DD");
        packages[i].time = moment().format("hh:mm");
        if (packages[i].payment_option === "collection") {
        ...
        if (customer === null) {
          ...
          await Customer.findOneAndUpdate(
            { seller: req.user._id },
            {
              rent_shelf_package_count: parseInt(
                customer.rent_shelf_package_count + 1
              ),
              total_package_count: parseInt(customer.total_package_count + 1),
            },
            { new: true, useFindAndModify: false }
      ...
      for (let i = 0; i < packages.length; i++) {
        let business = await Bussiness.findById(packages[i].businessId);
        let agent = await AgentDetails.findOne({
          _id: packages[i].senderAgentID,
        });
        let agent2 = await AgentDetails.findOne({
          _id: packages[i].receieverAgentID,
        });
        let newPackageCount = 1;
        ...
        packages[i].createdBy = req.user._id;
        packages[i].receipt_no = `${
          agent.prefix ? agent.prefix : "PMT-"
        }${newPackageCount}`;
        ...
        if (agent.hasShelf && agent2.hasShelf) {
          await new Track_agent_packages({
            ...
            descriptions: [
              {
                time: moment(),
                descriptions: `Pkg ${packages[i].receipt_no} created by ${business.name} and dropped at ${agent.business_name} for sorting`,
              },
            ],
            reciept: savedPackage.receipt_no,
          }).save();
        ...
        if (customer === null) {
          await new Customer({
            ...
            customer_phone_number: packages[i].customerPhoneNumber,
            total_package_count: 1,
            agent_package_count: 1,
          }).save();
        } else {
          await Customer.findOneAndUpdate(
            { seller: req.user._id },
            {
              agent_package_count: parseInt(customer.agent_package_count + 1),
              total_package_count: parseInt(customer.total_package_count + 1),
            },
            { new: true, useFindAndModify: false }
          );
...
router.get(
  "/rent-shelf-package-narations/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let narations = await rentshelfNarations
        .findOne({ package: req.params.id })
        .sort({ createdAt: -1 });
      return res.status(200).json(narations);
    } catch (error) {
      console.log(error);
      return res
        .status(400)
...
router.post("/package/delivery-charge", async (req, res) => {
  try {
    ...
    let recieverZone = await AgentLocation.findOne({
      _id: sender?.location_id,
    }).populate("zone");
    let senderZone = await AgentLocation.findOne({
      _id: receiver?.location_id,
    }).populate("zone");
...
router.get("/agents/test/viaob/hgfh", async (req, res) => {
  try {
    ...
    const sender = await AgentDetails.findOne({ _id: senderAgentID }).populate(
      "location_id"
    );

    const receiver = await AgentDetails.findOne({
      _id: receieverAgentID,
...
router.put(
  "/rent-shelf/package/:id/:state",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let package = await Rent_a_shelf_deliveries.findById(
        req.params.id
      ).populate("businessId");
      ...
      let seller = global.sellers?.find(
        (sel) => sel.seller === `${package.createdBy}`
      )?.socket;
      if (seller) {
        switch (state) {
          case "request":
            await new Notification({
              dispachedTo: package.createdBy,
              receipt_no: `${package.receipt_no}`,
              p_type: 3,
              s_type: 1,
              descriptions: ` Package #${package.receipt_no}  created`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
              .sort({ createdAt: -1 })
              .limit(9);
            global.io
              .to(seller)
              .emit("change-state", { notifications: notefications });
          ...
          case "picked-from-seller":
            await new Notification({
              ...
              p_type: 3,
              s_type: 2,
              descriptions: ` Package #${package.receipt_no}  picked from sender`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "collected":
            await new Notification({
              ...
              p_type: 3,
              s_type: 11,
              descriptions: ` Package #${package.receipt_no}  collected`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "rejected":
            await new Notification({
              ...
              p_type: 3,
              s_type: 0,
              descriptions: ` Package #${package.receipt_no}  been assigned to a rider from the warehouse`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "early_collection":
            await new Notification({
              ...
              p_type: 3,
              s_type: 12,
              descriptions: ` Package #${package.receipt_no}  booked for early collection`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
      ...
      if (req.params.state === "rejected") {
        ...
        let reject = await new Reject({
          package: req.params.id,
          reject_reason: req.body.rejectReason,
        }).save();
        await Rent_a_shelf_deliveries.findOneAndUpdate(
          { _id: req.params.id },
          { state: req.params.state, rejectedId: reject._id },
          { new: true, useFindAndModify: false }
        );
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg ${package.receipt_no} was rejected by ${auth?.name} because ${req.body.rejectReason} `,
          },
        ];

        await Track_rent_a_shelf.findOneAndUpdate(
          { package: req.params.id },
          { descriptions: new_des, rejectedAt: Date.now() },
          { new: true, useFindAndModify: false }
        );
      }
      if (req.params.state === "picked-from-seller") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg ${package.receipt_no} drop-off confirmed by ${auth?.name} `,
          },
        ];
        await Track_rent_a_shelf.findOneAndUpdate(
          { package: req.params.id },
        ...
        const textbody = {
          address: Format_phone_number(`${package.customerPhoneNumber}`),
          Body: `Hello  ${package.customerName}, Collect parcel ${package.receipt_no} from ${package?.businessId?.name} at Philadelphia house Track now:  pickupmtaani.com
      `,
        };
        ...
        await new Commision({
          agent: req.user._id,
          rent_shelf: req.params.id,
          commision: 0.1 * parseInt(payments),
        }).save();
      }

      if (req.params.state === "collected" && package.booked === true) {
        let collector = await Collected.findOneAndUpdate(
          { package: req.params.id },
          {
        ...
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg ${package.receipt_no} was given out to ${
              collector.collector_name
            } of phone No 0${collector.substring(1, 4)}xxx xxxx by  ${
              auth?.name
            } `,
          },
        ];
      ...
      if (req.params.state === "collected" && package.booked !== true) {
        ...
        let new_des = [
          ...narration.descriptions,
          {
            ...
            desc: `Pkg ${package.receipt_no} was given out to ${
              collector.collector_name
            } of phone No 0${collector.collector_phone_number.substring(
              1,
              4
            )}xxx xxxx by  ${auth?.name}  `,
          },
        ];
        await Track_rent_a_shelf.findOneAndUpdate(
          { package: req.params.id },
      ...
      if (req.params.state === "early_collection") {
        ...
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg ${package.receipt_no} was booked ${
              req.body.collector_name
            } of phone No 0${req.body.substring(1, 4)}xxx xxxx by  ${
              auth?.name
            }  `,
          },
        ];
        ...
        await Rent_a_shelf_deliveries.findOneAndUpdate(
          { _id: req.params.id },
          { booked: true, booked: booked, descriptions: new_des },
          { new: true, useFindAndModify: false }
        );
      ...
      if (req.params.state === "pending-agent") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg ${package.receipt_no} is transfared to Pickup agent`,
          },
        ];
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      agent_packages = await Rent_a_shelf_deliveries.find({
        location: id,
        state: req.params.state,
      })
        .sort({ createdAt: -1 })
    ...
    try {
      ...
      agent_packages = await Rent_a_shelf_deliveries.find({
        ...
        $or: [
          ...
          { receipt_no: searchKey },
          { customerPhoneNumber: searchKey },
        ],
      })
        .sort({ createdAt: -1 })
        .limit(100)
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.searchKey) {
        var searchKey = new RegExp(`${req.query.searchKey}`, "i");
        tra = await Track_rent_a_shelf.find({ $or: [{ reciept: searchKey }] })
          .sort({ createdAt: -1 })
          .limit(100)
          .populate("package");
        ...
        packages = await Track_rent_a_shelf.find()
          .sort({ createdAt: -1 })
          .limit(100)
          .populate({
            path: "package",
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.searchKey) {
        var searchKey = new RegExp(`${req.query.searchKey}`, "i");
        tra = await Track_rent_a_shelf.findOne({
          package: req.params.id,
          $or: [{ reciept: searchKey }],
        })
          .sort({ createdAt: -1 })
          .limit(100)
          .populate("package");
        ...
        packages = await Track_rent_a_shelf.findOne({ package: req.params.id })
          .sort({ createdAt: -1 })
          .limit(100)
          .populate("package")
          // .populate("collectedby")
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      let dropped = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "picked-from-seller",
      });
      let unavailable = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "unavailable",
      });
      let picked = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "picked",
      });
      let request = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "request",
      });
      let collected = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "collected",
      });
      let rejected = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "rejected",
      });
      let onTransit = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "on-transit",
      });
      let cancelled = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "cancelled",
      });
      let droppedToagent = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "dropped-to-agent",
      });
      let assigned = await Rent_a_shelf_deliveries.find({
        location: id,
        state: "assigned",
      });
      let agentOrderRequest = await Sent_package.find({
        payment_status: "paid",
        state: "pending-agent",
      })
        .sort({ createdAt: -1 })
        .limit(100);
      let earlyOrderRequest = await Rent_a_shelf_deliveries.find({
        location: id,
        $or: [{ state: "early_collection" }],
      });
      let doorSteporderRequest = await Door_step_Sent_package.find({
        $or: [{ payment_status: "paid" }, { payment_status: "to-be-paid" }],
        state: "pending-doorstep",
      })
        .sort({ createdAt: -1 })
        .limit(100)
        ...
        .populate("businessId")
        .populate("agent");
      let pickedfromSender = await Rent_a_shelf_deliveries.find({
        location: id,
        $or: [{ state: "picked-from-seller" }],
      });
...
router.get(
  "/rent-rider-shelf-package-count",
  [authMiddleware, authorized],
  async (req, res) => {
    let agent = await AgentUser.findOne({ user: req.user._id });
    const agentDetaoils = await AgentDetails.findOne({ user: req.user._id });
...
router.get("/packages", async (req, res) => {
  try {
    ...
    if (req.query.state === "all") {
      agent_packages = await Sent_package.find()
        .sort({ createdAt: -1 })
      ...
      agent_packages = await Sent_package.find({ state: req.query.state })
        .sort({ createdAt: -1 })
        .limit(limit)
        .populate("assignedTo", "name phone_number");
      doorstep_packages = await Door_step_Sent_package.find({
...
router.get(
  "/user-door-step-package/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      var searchKey = new RegExp(`${req.query.searchKey}`, "i");
      let packages = await Door_step_Sent_package.findOne({
...
router.get(
  "/user-rent_shelf-package/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      var searchKey = new RegExp(`${req.query.searchKey}`, "i");
      let packages = await Rent_a_shelf_deliveries.findOne({
        $or: [{ state: "picked-from-sender" }],
        createdBy: req.user._id,
        businessId: req.params.id,
        $or: [
          { packageName: searchKey },
...
router.get(
  "/user-rent-shelf-package-search/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      var searchKey = new RegExp(`${req.query.searchKey}`, "i");
      let packages = await Rent_a_shelf_deliveries.find({
        createdBy: req.user._id,
        businessId: req.params.id,
        $or: [
          { packageName: searchKey },
...
router.get(
  "/my-order-rent-shelf-packages/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      const { state } = req.query;
    ...
    try {
      ...
      if (req.query.searchKey) {
        ...
        agent_packages.created = await Sent_package.findOne({
          ...
          .populate("agent")
          .limit(100);
        agent_packages.dropped = await Sent_package.findOne({
          state: "picked-from-sender",
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .populate("agent")
          .limit(100);
        agent_packages.transit = await Sent_package.findOne({
          $or: [
            { state: "on-transit" },
          ...
          .populate("agent")
          .limit(100);
        agent_packages.warehouse = await Sent_package.findOne({
        ...
        agent_packages.delivered = await Sent_package.findOne({
          ...
          // .populate("agent")
          .limit(100);
        agent_packages.collected = await Sent_package.findOne({
          $or: [{ state: "collected" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
        ...
        doorstep_packages.created = await Door_step_Sent_package.find({
          state: "request",
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        doorstep_packages.dropped = await Door_step_Sent_package.findOne({
          state: "picked-from-sender",
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        doorstep_packages.transit = await Door_step_Sent_package.findOne({
          $or: [
            { state: "on-transit" },
            { state: "warehouse-transit" },
            { state: "assigned" },
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        doorstep_packages.warehouse = await Door_step_Sent_package.findOne({
          $or: [{ state: "recieved-warehouse" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
        ...
        doorstep_packages.delivered = await Door_step_Sent_package.findOne({
          $or: [{ state: "dropped-to-agent" }, { state: "delivered" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        doorstep_packages.collected = await Door_step_Sent_package.findOne({
          $or: [{ state: "collected" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
        ...
        // );
        shelves.created = await Rent_a_shelf_deliveries.findOne({
          state: "request",
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        shelves.dropped = await Rent_a_shelf_deliveries.findOne({
          $or: [{ state: "picked-from-seller" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
          ...
          .sort({ createdAt: -1 })
          .limit(100);
        shelves.collected = await Rent_a_shelf_deliveries.findOne({
          $or: [{ state: "collected" }],
          createdBy: req.user._id,
          businessId: req.params.id,
          $or: [
        ...
        agent_packages.created = await Sent_package.countDocuments({
          state: { $in: ["request","pending-shelf-agent","pending-stock-agent"] },
          businessId: req.params.id,
        })
       
        agent_packages.dropped = await Sent_package.countDocuments({
          state: "picked-from-sender",
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
          
        agent_packages.transit = await Sent_package.countDocuments({
          $or: [
            { state: "on-transit" },
            { state: "warehouse-transit" },
            { state: "assigned" },
        ...
        agent_packages.warehouse = await Sent_package.countDocuments({
          $or: [{ state: "recieved-warehouse" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
        ...
        agent_packages.delivered = await Sent_package.countDocuments({
          $or: [{ state: "dropped-to-agent" }, { state: "delivered" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
     
        agent_packages.collected = await Sent_package.countDocuments({
          $or: [{ state: "collected" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
        ...
        doorstep_packages.created = await Door_step_Sent_package.countDocuments({
          state: { $in: ["request","pending-shelf-doorstep","pending-stock-doorstep"] },
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
        
        doorstep_packages.dropped = await Door_step_Sent_package.countDocuments({
          state: "picked-from-sender",
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
         
        doorstep_packages.transit = await Door_step_Sent_package.countDocuments({
          $or: [
            { state: "on-transit" },
            { state: "warehouse-transit" },
            { state: "assigned" },
        ...
        doorstep_packages.warehouse = await Door_step_Sent_package.countDocuments({
          $or: [{ state: "recieved-warehouse" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
        ...
        doorstep_packages.delivered = await Door_step_Sent_package.countDocuments({
          $or: [{ state: "dropped-to-agent" }, { state: "delivered" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
          
        doorstep_packages.collected = await Door_step_Sent_package.countDocuments({
          $or: [{ state: "collected" }],
          // createdBy: req.user._id,
          businessId: req.params.id,
        })
...
router.get(
  "/booked-for-ealy-collection",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      const booked = await Rent_a_shelf_deliveries.find({
        $or: [{ state: "early_collection" }],
      })
        .sort({ createdAt: -1 })
        .limit(100)
...
router.get("/package/:id", async (req, res) => {
  try {
    ...
    let package = await Door_step_Sent_package.findById(req.params.id)
      .populate("agent")
      .populate({
        path: "package",
        populate: {
    ...
    const sender = await AgentDetails.findOne({
      $or: [
        { _id: package?.senderAgentID },
        { _id: package?.receieverAgentID },
        { _id: package?.agent },
        { _id: rent?.location },
      ],
    });
...
router.get("/collectors", async (req, res) => {
  try {
    ...
    let users;
    if (type === "rent") {
      users = await Collected.find({ type: "rent" })
        .populate("package2", "packageName receipt_no")
        .populate("dispatchedBy", "name")
        .sort({ createdAt: -1 });
    }
    if (type === "doorstep") {
      users = await Collected.find({ type: "doorstep" })
        .populate("package3", "packageName receipt_no")
        .populate("dispatchedBy", "name")
        .sort({ createdAt: -1 });
    }
    if (type === "agent") {
      users = await Collected.find({ type: "agent" })
        .populate("package1", "packageName receipt_no")
        .populate("dispatchedBy", "name")
        .sort({ createdAt: -1 });
    }
