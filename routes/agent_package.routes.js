...
router.put(
  "/agent/package-update/:id",
  [authMiddleware, authorized],
  async (req) => {
    try {
      await Sent_package.findOneAndUpdate({ _id: req.params.id }, req.body, {
        new: true,
    ...
    try {
        ...
        switch (state) {
          ...
          case "recieved-warehouse":
            await new Notification({
              ...
              p_type: 1,
              s_type: 6,
              descriptions: ` Package #${package.receipt_no} recieved at the warehouse`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "assigned-warehouse":
            await new Notification({
              ...
              p_type: 1,
              s_type: 7,
              descriptions: ` Package #${package.receipt_no}  assigned to a new rider`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "warehouse-transit":
            await new Notification({
              ...
              p_type: 1,
              s_type: 8,
              descriptions: ` Package #${package.receipt_no}  dispatched from warehouse`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "dropped-to-agent":
            await new Notification({
              ...
              p_type: 1,
              s_type: 9,
              descriptions: ` Package #${package.receipt_no}  dropped to the recieving agent`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "declined":
            await new Notification({
              ...
              p_type: 1,
              s_type: 11,
              descriptions: ` Package #${package.receipt_no}  rejected`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "droped":
            await new Notification({
              ...
              p_type: 1,
              s_type: 5,
              descriptions: ` Package #${package.receipt_no}  dropped to the the warehouse`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "on-transit":
            await new Notification({
              ...
              p_type: 1,
              s_type: 4,
              descriptions: ` Package #${package.receipt_no} on trans-it`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
          ...
          case "assigned":
            await new Notification({
              ...
              p_type: 1,
              s_type: 3,
              descriptions: ` Package #${package.receipt_no}  been assigned to a rider`,
            }).save();
            notefications = await Notification.find({
              dispachedTo: package.createdBy,
            })
      ...
      if (req.params.state === "assigned") {
        try {
          ...
          if (
            package.senderAgentID._id.toString() ===
            package.receieverAgentID.toString()
          ) {
            await Sent_package.findOneAndUpdate(
              { _id: req.params.id },
              { state: "delivered" },
            ...
            let new_des = [
              ...
              {
                time: Date.now(),
                desc: `Drop off confirmed by ${auth.name} at Philadelphia 4th floor awaiting to be assigned to ${rider?.name} `,
              },
            ];
            await Track_agent_packages.findOneAndUpdate(
              { package: req.params.id },
          ...
          if (agent?.hasShelf) {
            ...
            let new_description = [
              ...
              {
                time: Date.now(),
                desc: `Pkg ${package.receipt_no}  dropped at Phildelphia sorting area`,
              },
            ];
            await Track_agent_packages.findOneAndUpdate(
              { package: req.params.id },
            ...
            await new Narations({
              package: req.params.id,
              state: req.params.state,
              descriptions: `Package dropped to agent(${package.receieverAgentID.business_name})`,
            }).save();
            let payments = getRandomNumberBetween(100, 200);
            await new Commision({
              agent: req.user._id,
              agent_package: req.params.id,
            ...
            let new_description = [
              ...
              {
                time: Date.now(),
                desc: `Drop off confimed  by ${auth?.name} at  ${sender.business_name} waiting for rider to collect`,
              },
            ];
            await Track_agent_packages.findOneAndUpdate(
              { package: req.params.id },
      ...
      if (req.params.state === "on-transit") {
        ...
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg collected by rider ${rider.name} waiting to be dropped at sorting, Phildelphia Hse`,
          },
        ];
        ...
        await new Narations({
          package: req.params.id,
          state: req.params.state,
          descriptions: `package delivered to agent name(${package.receieverAgentID.business_name})`,
        }).save();
      }
      if (req.params.state === "dropped") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg dropped by rider ${rider.name} at sorting, Phildelphia Hse`,
          },
        ];
        await Track_agent_packages.findOneAndUpdate(
          { package: req.params.id },
      ...
      if (req.params.state === "recieved-warehouse") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg  recieved at sorting  philadelphia and awaiting to  be assigned to rider going to receieverAgentID location, ${reciever.business_name} `,
          },
        ];
        ...
        await new Narations({
          package: req.params.id,
          state: req.params.state,
          descriptions: `Package dropped to warehouse`,
        }).save();
      }
      if (req.params.state === "assigned-warehouse") {
        await new Narations({
          package: req.params.id,
          state: req.params.state,
          descriptions: `Package package assigned rider`,
        }).save();
        await new Rider_Package({
          package: req.params.id,
          rider: req.query.rider,
        }).save();
        await Sent_package.findOneAndUpdate(
          { _id: req.params.id },
          { assignedTo: req.query.rider },
          { new: true, useFindAndModify: false }
        );
        ...
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg  assigned  by philadelphia sorting to ${newrider.name} heading to ${reciever.business_name}`,
          },
        ];
        let v = await Track_agent_packages.findOneAndUpdate(
          { package: req.params.id },
          {
            reAssigned: {
      ...
      if (req.params.state === "warehouse-transit") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg  accepted by ${recRider.name}  waiting drop off to destination location, ${reciever.business_name} `,
          },
        ];
        await Track_agent_packages.findOneAndUpdate(
          { package: req.params.id },
      ...
      if (req.params.state === "dropped-to-agent") {
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Pkg delivered by ${recRider.name} at destination agent, ${reciever.business_name}`,
          },
        ];
        await Track_agent_packages.findOneAndUpdate(
          { package: req.params.id },
      ...
      if (req.params.state === "delivered") {
        ...
        let new_des = [
          ...
          {
            time: Date.now(),
            desc: `Agent ${auth.name}  confirms arrival of package at the destination agent, ${reciever.business_name}`,
          },
        ];
      ...
      if (req.params.state === "collected") {
        try {
          ...
          let new_des = [
            ...
            {
              time: Date.now(),
              desc: `Pkg given out to ${collector.collector_name} of ID no ${
                collector.collector_national_id
              } phone No 0${collector.collector_phone_number.substring(
                1,
                4
              )}xxx xxxx   `,
            },
          ...
          await new Narations({
            package: req.params.id,
            state: req.params.state,
            descriptions: `Package collected by customer.`,
          }).save();
        } catch (error) {
          console.log(error);
        }
    ...
    try {
      ...
      if (req.query.searchKey) {
        agent_packages = await Sent_package.find({
          ...
          $or: [{ packageName: searchKey }, { receipt_no: searchKey }],
        })
          .sort({ createdAt: -1 })
          .limit(100)
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i < packages.length; i++) {
        agents_count[packages[i].senderAgentID.toString()] = agents_count[
          packages[i].senderAgentID.toString()
        ]
          ? [
              ...agents_count[packages[i].senderAgentID.toString()],
              packages[i]._id,
            ]
          : [packages[i]._id];
      }
      return res.status(200).json(agents_count);
    } catch (error) {
      console.log(error);
      return res
        .status(400)
...
router.put(
  "/wh-agent-pick-package/:id/:state",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let package = await Sent_package.findById(req.params.id).populate(
      ...
      if (agent?.hasShelf) {
        let packages = await Sent_package.findOneAndUpdate(
          { _id: req.params.id },
          { state: "delivered" },
          { new: true, useFindAndModify: false }
        );
        ...
        const textbody = {
          address: Format_phone_number(`${package.customerPhoneNumber}`),
          Body: `Hi ${package.customerName}\nYour Package ${package.receipt_no} has been delivered to ${package?.senderAgentID?.business_name} and will be shipped to in 24hrs `,
        };
        await SendMessage(textbody);
        await new Narations({
          package: req.params.id,
          state: req.params.state,
          descriptions: `Your Package ${package.receipt_no} has been delivered to ${package?.senderAgentID?.business_name}`,
        }).save();
        let payments = getRandomNumberBetween(100, 200);
        await new Commision({
          agent: req.user._id,
...
router.put(
  "/wh-agentassign-package/:id/:state",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let { rider } = req.query;
      let packages = await Sent_package.findOneAndUpdate(
        { _id: req.params.id },
        { state: req.params.state, assignedTo: rider },
        { new: true, useFindAndModify: false }
      );
...
router.get(
  "/reciever-agents-rider-packages",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let { state } = req.query;
      ...
      for (let i = 0; i < packages.length; i++) {
        agents_count[packages[i].receieverAgentID.toString()] = agents_count[
          packages[i].receieverAgentID.toString()
        ]
          ? [
              ...agents_count[packages[i].receieverAgentID.toString()],
              packages[i]._id,
            ]
          : [packages[i]._id];
      }
...
router.get(
  "/agents-wh-recieved-warehouse-package",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      // let agent = await AgentUser.findOne({ user: req.user._id })
...
router.get(
  "/agents-rider-package-count",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let OnTransit = await Sent_package.find({
        payment_status: "paid",
        state: "on-transit",
        assignedTo: req.user._id,
      });
      let assigned = await Sent_package.find({
        payment_status: "paid",
        state: "assigned",
        assignedTo: req.user._id,
      });
      let warehouseTransit = await Sent_package.find({
        payment_status: "paid",
        state: "warehouse-transit",
        assignedTo: req.user._id,
      });
      let assignedWarehouse = await Sent_package.find({
        payment_status: "paid",
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.searchKey) {
        ...
        agent_packages = await Sent_package.find({
          businessId: id,
          updatedAt: { $lte: moment().subtract(4, "days").toDate() },
          state: { $ne: "collected" },
          createdBy: req.user._id,
          $or: [
            { packageName: searchKey },
...
router.get(
  "/agent/switchboard-search",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      var searchKey = new RegExp(`${req.query.searchKey}`, "i");
      let agent_packages = await Track_agent_packages.find({
        reciept: searchKey,
      })
        .sort({ createdAt: -1 })
        .limit(100)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      agent_packages = await Sent_package.find({
        state: req.params.state,
        $or: [{ senderAgentID: id }, { receieverAgentID: id }],
        $or: [{ packageName: searchKey }, { receipt_no: searchKey }],
      })
        .sort({ createdAt: -1 })
        .limit(100)
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        agents_count[packages[i]?.businessId?.toString()] = agents_count[
          packages[i]?.businessId?.toString()
        ]
          ? {
              packages: [
                ...agents_count[packages[i]?.businessId?.toString()]?.packages,
                packages[i]._id,
              ],
              name: package.businessId.name,
            }
...
router.get(
  "/agent-package-track/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let narations = await Track_agent_packages.find({
        package: req.params.id,
      }).sort({ createdAt: -1 });
...
router.get(
  "/agent-package-narations/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let narations = await Narations.find({ package: req.params.id }).sort({
        createdAt: -1,
      });
      return res.status(200).json(narations);
    } catch (error) {
...
router.get(
  "/agents-packages-recieved-warehouse",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let packages = await Sent_package.find({
        state: "recieved-warehouse",
        $or: [
          { receieverAgentID: req.query.agent },
          { senderAgentID: req.query.agent },
        ],
      })
        .sort({ createdAt: -1 })
        .limit(100)
...
router.get(
  "/receiever-agent-packages/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      const { state } = req.query;
      let packages = await Sent_package.find({
        receieverAgentID: req.params.id,
        state: state,
      })
        .populate("createdBy", "l_name f_name phone_number")
        .populate({
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let assigneWarehouse = await Sent_package.find({
        agent: id,
      ...
      let unavailable = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "unavailable",
      });
      let picked = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "picked",
      });
      let request = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "request",
      });
      let delivered = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "delivered",
      });
      let collected = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "collected",
      });
      let rejected = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "rejected",
      });
      ...
      let cancelled = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "cancelled",
      });
      let droppedToagent = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
      ...
      let recievedWarehouse = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
        state: "recieved-warehouse",
      });
      let pickedfromSender = await Sent_package.find({
        agent: id,
        $or: [{ payment_status: "paid" }, { payment_status: "paid" }],
...
router.get(
  "/web/all-agent-packages/packages",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let packages = await Sent_package.find()
    ...
    try {
      ...
      if (period === 0 || period === undefined || period === null) {
        ...
        packages = await Sent_package.find({
          ...
          receieverAgentID: id,
          state: state,
          updatedAt: { $gte: moment().subtract(period, "days").toDate() },
          $or: [{ packageName: searchKey }, { receipt_no: searchKey }],
        })
          .populate("createdBy", "l_name f_name phone_number")
          .populate("senderAgentID")
...
router.get(
  "/single-business-agent-packages",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      const { agent, state, id } = req.query;
...
router.get("/agent/track/packages", async (req, res) => {
  try {
    ...
    if (req.query.searchKey) {
      var searchKey = new RegExp(`${req.query.searchKey}`, "i");
      packages = await Track_agent_packages.find({
...
router.get(
  "/agent/track/packages/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      let packages;

      packages = await Track_agent_packages.findOne({ package: req.params.id })
        .sort({ createdAt: -1 })
        .limit(100)
