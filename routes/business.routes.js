...
const {
  validateBusinesInput,
} = require("../va;lidations/business.validations");
  ...
  async (req, res) => {
    try {
      ...
      if (Exists) {
        return res
          .status(400)
          .json({ message: "You Already added this business !!" });
      }
...
router.get("/busi/:id", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Businesses Fetched Successfully !!", bussiness });
  } catch (error) {
...
router.get("/single-business/:id", async (req, res) => {
  try {
    const bussiness = await Business.findById(req.params.id)
      .populate("details")
...
router.put("/business/:id", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    const Edited = await Business.findOneAndUpdate(
      { _id: req.params.id },
      body,
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const Edited = await Business.findOneAndUpdate(
        { _id: req.params.id },
        { has_shelf: true, request_shelf: false },
        { new: true, useFindAndModify: false }
      );
      return res.status(200).json({ message: "Edited successfully", Edited });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (body.choice) {
        const Edited = await Business.findOneAndUpdate(
          { _id: req.params.id },
          { shelf_location: body.shelf_location, request_shelf: true },
          { new: true, useFindAndModify: false }
        );
        return res.status(200).json({ message: "Edited successfully", Edited });
      } else {
  ...
  async (req, res) => {
    try {
      ...
      if (body.choice) {
        ...
        const Edited = await Business.findOneAndUpdate(
          { _id: req.params.id },
          { shelf_location: req.params.new_location, request_shelf: true },
          { new: true, useFindAndModify: false }
        );
        return res.status(200).json({ message: "Edited successfully", Edited });
      } else {
...
router.get(
  "/rent-a-shelf-agents/:id/business",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "fetched successfully", bussiness });
    } catch (error) {
      return res
        .status(400)
        .json({ success: false, message: "operation failed ", error });
    ...
    try {
      ...
      if (req.file) {
        ...
        const update = await Business.findOneAndUpdate(
          { _id: req.params.id },
          { logo: url + "/uploads/bussiness_logo/" + req.file.filename },
          { new: true, useFindAndModify: false }
        );
        return res
          .status(200)
          .json({ message: "Logo Updated Successfully", update });
      } else {
        const url = req.protocol + "://" + req.get("host");
        const update = await Business.findOneAndUpdate(
          { _id: req.params.id },
        ...
        return res
          .status(200)
          .json({ message: "Logo deleteed Successfully", update });
      }
      // }
    } catch (error) {
      return res
...
router.post("/business/:id/details", async (req, res) => {
  try {
    ...
    if (exists) {
      const Edited = await BUssinessDeatails.findOneAndUpdate(
      ...
      return res
        .status(200)
        .json({ message: "Saved successfully", newDetails });
    }
  } catch (error) {
    console.log(error);
    return res
