  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        if (rider) {
          const Update = await Rider.findOneAndUpdate(
            { user: req.user._id },
...
router.post(
  "/update-rider-id-front",
  [authMiddleware, authorized],
  uploadrider_id_front.single("rider_id_front"),
  async (req, res) => {
    try {
      const url = req.protocol + "://" + req.get("host");
      const rider = await Rider.findOne({ user: req.user._id });
      ...
      if (req.file) {
        if (rider) {
          ...
          const Update = await Rider.findOneAndUpdate(
            { user: req.user._id },
            { rider_id_front: req.body.rider_id_front },
            { new: true, useFindAndModify: false }
          );
          ...
          const Update = await new Rider({
            rider_avatar: req.body.rider_id_front,
            user: req.user._id,
          }).save();
          user.bike_reg_plate = rider?.bike_reg_plate;
          user.rider_avatar = rider?.rider_avatar;
...
router.post(
  "/update-rider-licence",
  [authMiddleware, authorized],
  uploadrider_licence_photo.single("rider_licence_photo"),
  async (req, res) => {
    try {
      const url = req.protocol + "://" + req.get("host");
      const rider = await Rider.findOne({ user: req.user._id });
      ...
      if (req.file) {
        if (rider) {
          const body = req.body;
          body.rider_licence_photo =
            url + "/uploads/licence/" + req.file.filename;

          const Update = await Rider.findOneAndUpdate(
            { user: req.user._id },
            { rider_licence_photo: req.body.rider_licence_photo },
            { new: true, useFindAndModify: false }
          );
          user.bike_reg_plate = rider?.bike_reg_plate;
          user.rider_avatar = rider?.rider_avatar;
          ...
          const Update = await new Rider({
            rider_avatar: req.body.rider_licence_photo,
            user: req.user._id,
          }).save();
          user.bike_reg_plate = Update.bike_reg_plate;
          user.rider_avatar = Update.rider_avatar;
...
router.post("/update-rider", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    const body = req.body;
    if (rider) {
      await Rider.findOneAndUpdate({ user: req.user._id }, body, {
        new: true,
      ...
      return res
        .status(200)
        .json({ success: true, message: "Rider Updated Successfully ", user });
    } else {
...
router.put("/assign-agent-rider/:id/:agent", async (req, res) => {
  try {
    ...
    await new RiderRoutes({
      agent: req.params.agent,
      rider: req.params.id,
    }).save();

    const aggent = await AgentUser.findOne({
      agent: req.params.agent,
      role: "rider",
    });
    if (aggent) {
      await AgentUser.findOneAndUpdate(
        { agent: req.params.agent, role: "rider" },
        { user: req.params.id },
        { new: true, useFindAndModify: false }
      );
    } else {
...
router.post("/rider", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Rider Added successfully", saved: saved });
  } catch (error) {
    return res
      .status(400)
      .json({ success: false, message: "operation failed ", error });
...
router.get("/agents/:id", [authMiddleware, authorized], async (req, res) => {
  try {
    const riders = await RiderRoutes.find({ rider: req.params.id }).populate(
      "agent"
    );
...
router.get(
  "/riders-agents-package-count/:id",
  [authMiddleware, authorized],
  async (req, res) => {
    try {
      const { state } = req.query;
      const agents = await RiderRoutes.find({ rider: req.params.id }).populate(
        "agent",
        "business_name rider"
      );
      ...
      for (let i = 0; i < agents.length; i++) {
        let packages = await Sent_package.find({
          ...
          receieverAgentID: agents[i].agent?._id,
          state: state,
        });
      ...
      for (let j = 0; j < agents.length; j++) {
        let packages = await Sent_package.find({
          assignedTo: req.params.id,
          senderAgentID: agents[j].agent?._id,
          state: state,
        });
      ...
      return res
        .status(200)
        .json({ reciever_agents_count, sender_agents_count });
    } catch (error) {
      console.log(error);
      return res
        .status(400)
...
router.put("/agent/package1/:id/assign-rider", async (req, res) => {
  ...
  try {
    await new Rider_Package({
      ...
      rider: "632181644f413c3816858218",
    }).save();
    console.log("riders");
    await Sent_package.findOneAndUpdate(
      { _id: req.params.id },
      { state: "assigned", assignedTo: "632181644f413c3816858218" },
      { new: true, useFindAndModify: false }
    );
    console.log("here");
    return res.status(200).json({ message: "Sucessfully" });
...
router.post("/assign-riders", async (req, res) => {
  try {
    ...
    const exists = await Conversation.findOne({
      members: {
        $all: [req.body.rider, packOwner.createdBy],
      },
    });
    ...
    if (exists) {
      await Conversation.findOneAndUpdate(
        { _id: exists._id },
        {
          updated_at: new Date(),
          last_message:
            "Hi  been assigned your package kindly feel free to chat",
        },
        { new: true, useFindAndModify: false }
      );
      await new Message({
        conversationId: exists._id,
        sender: req.body.rider,
        text: `Hi  been assigned your package kindly feel free to chat`,
      }).save();
      await Sent_package.findOneAndUpdate(
        { _id: req.body.package },
        { state: "assigned", assignedTo: req.body.rider },
        { new: true, useFindAndModify: false }
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      const packages = await Rider_Package.find({
        rider: req.user._id,
      }).populate({
        path: "package",
        populate: [
...
router.post("/tracking/:id", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    console.log("RIDER TRACKING ERROR! ", error);
