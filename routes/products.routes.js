...
router.put("/product/:id", [authMiddleware, authorized], async (req, res) => {
  try {
    ...
    let v = await Stock.findOneAndUpdate({ product: req.params.id }, req.body, {
      new: true,
  ...
  async (req, res) => {
    try {
      ...
      fs.unlink(
        __dirname +
          "./../uploads/products/" +
          path.basename(prod.images[req.body.index]),
        async (err) => {
          if (err) throw err;
          ...
          return res
            .status(200)
            .json({ message: "successfully deleted file " });
        }
      );
      //
    } catch (error) {
