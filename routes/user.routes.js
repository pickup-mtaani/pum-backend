    ...
    if (userOBJ && userOBJ.activated === false) {
      if (userOBJ.role === "agent") {
        return res
          ...
          .json({
            message:
              "Your Account is not Activated kindly contact support for activation",
            userOBJ,
          });
      }
      return res
        ...
        .json({
          message:
            "Your Account is not Activated kindly enter the code sent to your phone via text message",
          userOBJ,
        });
    }
...
router.post("/social-login", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const token = await jwt.sign(
        { email: req.body.email, _id: savedUser._id },
        process.env.JWT_KEY
      );
      return res
        .status(200)
...
router.post("/register", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "User Saved Successfully !!", saved });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
...
router.post("/:id/resend-token", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: false, message: "Activation resent " });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
...
router.put("/reset-password", async (req, res) => {
  try {
    ...
    req.body.phone_number = await Format_phone_number(req.body.phone_number); //format the phone number
    let user =
      (await User.findOne({ phone_number: req.body.phone_number })) ||
      (await Rider.findOne({ phone_number: req.body.phone_number }));
  ...
  try {
    ...
    if (req.body.phone_number) {
      ...
      return res
        ...
        .json({
          success: true,
          message: `Email sent with a recovery code to ${req.body.email}`,
          userUpdate,
        });
    }
  } catch (error) {
...
router.put("/user/make-super/:id", async (req, res) => {
  try {
    let userObj = await Agent.findOneAndUpdate(
      { user: req.params.id },
      { isSuperAgent: true },
      { new: true, useFindAndModify: false }
    );
