  ...
  const Sellers_columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <>
          <select
            className=" bg-primary-500 w-38 mb-2 mx-2 rounded-md float-right h-10 flex justify-center items-center px-2 border-none"
            onChange={(e) => {
              setShow(true);
              setId(row._id);
          ...
          >
            ...
            {props.riders?.map((rider, i) => (
              <option key={i} value={rider?.user?._id}>
                {rider?.user?.name}
              </option>
            ))}
          </select>
  ...
  return (
    <Layout>
      ...
      <ConfirmModal
        ...
        show={show}
        Submit={async () => {
          await props.CollectDoorStep(id, "assigned-warehouse", rider);
          setShow(false);
