    ...
    <Layout>
      <div className="m-8 bg-white rounded pb-12">
        ...
        <div className="p-3 border-b-2 border-b-blue-500 font-semibold text-xl text-center">
          Assign Doorstep Packages to Rider
        </div>
        <div className="p-4 flex flex-wrap">
          {props?.riders?.map((rider) => (
            <RiderItem
              rider={rider}
              path={`/wahehouse/doorstep/assign-rider`}
            />
          ))}
        </div>
      </div>
...
export default connect(mapStateToProps, { get_riders })(AssignDoorstep);
