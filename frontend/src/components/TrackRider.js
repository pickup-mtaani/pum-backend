...
import { fetchpackages, rider_route } from "../redux/actions/riders.actions";
...
function Map(props) {
  ...
  const fetch = async () => {
    ...
    if (v.length > 0) {
      ...
      setPickUp({ lat: parseFloat(v[9].lat), lng: parseFloat(v[5].lng) });
  ...
  useEffect(() => {
    ...
    socket.emit("track_rider", {
      rider_id: location.state.id,
      user_id: "1322",
    });
