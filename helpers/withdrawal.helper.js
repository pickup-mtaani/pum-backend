...
const handleB2C = async (withdrawal_amount,withdrawal_phone,user,packages,business) => {

    try {
        const encode = new Buffer.from(`${process.env.MPESA_TRANSACTION_CONSUMER_KEY}:${process.env.MPESA_TRANSACTION_CONSUMER_SECRET}`).toString("base64")
        ...
        const query_response = await axios.post(process.env.MPESA_TRANSACTION_STATUS_URL, {
            Initiator: "pickupmtaani",
            ...
            // QueueTimeOutURL: "https://0f52-196-207-182-190.eu.ngrok.io/api/transaction_timeout_callback",
